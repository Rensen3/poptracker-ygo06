...
Yu-Gi-Oh! 2006_location = {}
Yu-Gi-Oh! 2006_location.__index = Yu-Gi-Oh! 2006_location
...
function YuGiOh2006_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        exit = Yu-Gi-Oh! 2006_location.new(exit)
    end
    if rule == nil then
        rule = always
    end
...
function YuGiOh2006_location:connect_two_ways(exit, rule)
...
function YuGiOh2006_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function YuGiOh2006_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function YuGiOh2006_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
